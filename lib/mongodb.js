...
MongoDB.prototype.updateAttributes = function updateAttrs(model, id, data, cb) {
  ...
  if (typeof this._models[model].model.getShardKeyName === 'function'){
    var shardKeyName = this._models[model].model.getShardKeyName() || undefined;
    if (shardKeyName && typeof this._models[model].model.getShardKeyValue === 'function'){
      query[shardKeyName] = this._models[model].model.getShardKeyValue();
    }
  }

  this.collection(model).findAndModify(query, [
    ['_id', 'asc']
